...
OPERATE_RET app_chat_bot_init(void)
{
...
#if defined(ENABLE_COMP_AI_DISPLAY) && (ENABLE_COMP_AI_DISPLAY == 1)
    TUYA_CALL_ERR_RETURN(avatar_ui_register());
#endif

    /* Register combo mode before ai_chat_init so it is in the mode list. */
    TUYA_CALL_ERR_RETURN(ai_mode_combo_register());

    AI_CHAT_MODE_CFG_T ai_chat_cfg = {
        .default_mode = AI_CHAT_MODE_COMBO, /* wake word + click + long-press PTT */
        .default_vol  = 70,
        .evt_cb       = __ai_chat_handle_event,
    };
    TUYA_CALL_ERR_RETURN(ai_chat_init(&ai_chat_cfg));

    /* Force combo on boot: KV may still hold HOLD/FREE from earlier runs. */
    {
        AI_CHAT_MODE_E cur_mode;
...
#if defined(ENABLE_COMP_AI_DISPLAY) && (ENABLE_COMP_AI_DISPLAY == 1)
    /* app_ui_action_register(); */ /* camera/album actions not used on desktop avatar */
#endif
    ...
    TUYA_CALL_ERR_RETURN(ui_mcp_tools_init());
    TUYA_CALL_ERR_RETURN(motion_mcp_init());
#endif

#if defined(ENABLE_MOTION_ENGINE) && (ENABLE_MOTION_ENGINE == 1)
    TUYA_CALL_ERR_RETURN(motion_engine_init());
#endif

#if defined(ENABLE_SERVO_TEST) && (ENABLE_SERVO_TEST == 1)
    TUYA_CALL_ERR_LOG(servo_test_start());
#endif
    ...
    ai_ui_disp_msg(AI_UI_DISP_STATUS, (uint8_t *)"Say NiHaoTuYa / Click",
                   strlen("Say NiHaoTuYa / Click"));
    ai_ui_disp_msg(AI_UI_DISP_EMOTION, (uint8_t *)"SLEEPY", strlen("SLEEPY"));
