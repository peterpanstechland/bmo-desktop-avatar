...
void user_main(void)
{
    ...
    if (OPRT_OK != tuya_authorize_read(&license)) {
        ...
        if ('\0' == license.uuid[0] || '\0' == license.authkey[0]) {
            PR_ERR("No license found. Flash one with `tos.py auth`, or set CONFIG_TUYA_OPENSDK_UUID and "
                   "CONFIG_TUYA_OPENSDK_AUTHKEY in your local app_default.config, otherwise the demo cannot work.\n \
