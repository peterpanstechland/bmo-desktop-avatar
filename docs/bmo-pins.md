...
| 红大圆（对话） | P12 | `ai_chat_button`：单击唤醒/打断/结束对话，长按对讲（listening 时改为结束对话），双击切模式，见下节。**注意：stock `TUYA_T5AI_CORE` 把该键注册在板载 P29，BMO 面板红键在 P12；需打 `patches/board_chat_button_p12.patch`（active-high / rising edge，与绿键同极性）。** |
...
底部常驻提示「按十字中设置闹钟/计时」。有闹钟或倒计时时，翻牌下方会显示状态行。

| 状态 | 按键 |
|------|------|
| 主页 | **十字中**进入设置菜单；左/右翻页、上/下调音量照常 |
| 菜单 | 上/下在「闹钟 / 计时器」间选择；中确认进入编辑；**绿键**关闭当前项（关闹钟或取消计时）；三角退出 |
| 编辑闹钟 | 翻牌显示目标 HH:MM；左右切「时/分」，上下调值，中保存；三角回菜单 |
| 编辑计时 | 翻牌显示 MM:SS；左右切「分/秒」，上下调值，中开始；三角回菜单 |

到点会跳回时钟页、播放提示音，并显示「按键关闭」；任意键关闭响铃。

也可用语音让智能体调 MCP：`pet.alarm.set` / `pet.alarm.clear` / `pet.timer.start` / `pet.timer.cancel`。

## 日历页（第 4 页 calendar）

左右翻页到日历后，默认键位不变：上/下音量、左/右翻页、十字中刷新、绿键刷新、三角回表情页。

要按天看日程时进入**浏览模式**（二级，对齐游戏页的吃键套路）：

| 状态 | 按键 |
|------|------|
| 未浏览 | 底部提示「按十字中进入浏览」；**十字中**进入；左/右翻页、上/下调音量照常 |
| 浏览中 | 底部提示「左右换天 · 三角退出」；左/右换天（今天起 4 天）；上/下仍调音量；**蓝三角**退出浏览并底部弹出「已退出浏览」；十字中 / 绿键刷新 |

浏览中右上角显示「选 M月D日」，右侧标题从「今日日程」变成对应日期；月历里选中日反色，今天若未选中则保留描边。飞书只同步 4 天，再往外按键会停在边界。

离开日历页（左/右翻走或三角回表情）会清掉浏览状态。

## Maker RSS 页（第 5 页 rss）

在日历与游戏之间。后台每 30 分钟拉一次 8 个源（Hackaday、CNX Software、Make:、Adafruit Blog、Adafruit Learn、Raspberry Pi News、Seeed Studio、Arduino Blog），每源最多 4 条标题+摘要，混排显示。正文保持英文；界面提示跟 UI 语言。绿键可手动刷新。

| 状态 | 按键 |
|------|------|
| 未浏览 | 底部提示「按十字中浏览 · 绿键刷新」；**十字中**进入浏览；左/右翻页、上/下调音量照常 |
| 浏览中 | 左右切换源（含「全部」）；上下选条目（反色）；**十字中**打开摘要；**蓝三角**退出浏览；绿键刷新 |
| 摘要详情 | 标题+源名+摘要；上下滚动正文；左右换上/下一条；**蓝三角**返回列表；绿键刷新 |

语音「打开资讯 / 新闻 / RSS」或 MCP `pet.screen.show_page` 的 `page=rss` 可直达。

## 小游戏页（第 6 页 games）

从 RSS 页再按 **RIGHT** 进入，或语音「打开游戏」。
...
## Settings 页（第 7 页 settings）

从游戏页再按 **RIGHT** 进入，或语音「打开设置 / Settings」。

本地 UI 语言（中/英）保存在 KV，**不会**切换涂鸦云端 ASR/TTS / 音色；语音对话语言由智能体提示词跟随用户说话语言。

| 行 | 内容 | 操作 |
|----|------|------|
| 语言 / Language | 显示中文或 English | 浏览中左右切换，立即重绘 |
| 固件版本 / Firmware | 只读 `PROJECT_VERSION` | — |
| 检查更新 / Check update | 触发云端静默升级查询 | 中键/绿键，toast 结果 |
| 更新 / Update | 若 OTA 已在进行则提示稍候；否则等同再检查一次 | 真正刷写仍靠云端推送 OTA |

| 状态 | 按键 |
|------|------|
| 未浏览 | 底部提示进入设置；**十字中**进入浏览；左/右翻页照常 |
| 浏览中 | 上下选行；语言行左右切换；中/绿执行；**蓝三角**退出浏览 |
