...
固件注册了 MCP 工具 `pet.calendar.add_event`，云端智能体发现后就能用，设备端不需要额外配置。跟它说「明天下午三点开会」「后天九点提醒我交周报」即可，写成功后屏幕会跳到日历页并弹一条「日程已添加」，日程列表随即刷新。

日期传的是**相对天数**（`day_offset`：今天 0、明天 1），不是绝对日期。云端智能体不一定知道今天几号，但设备有 NTP 校准过的时钟，所以让设备自己换算更稳。时间按本地时区解释，没说时长默认一小时。

提示词里已经写好了对应的用法说明，见 [agent-system-prompt.md](agent-system-prompt.md)「记日程」一条。

几点注意：

- 这是往日历里写事件，不是设闹钟，到点提醒由飞书 App 发。
- 机器人对日历只有 `reader` 时会失败，串口报 `add event rejected`，按上面的方法升到 `writer`。
