...
| Pages | Avatar, clock, weather, calendar (Feishu sync) |
| Voice | Tuya AI combo mode — wake word, click, long-press PTT |
| Buttons | D-pad (volume / page nav), centre refresh, SW1 mute, SW2 sysinfo, triangle home, green easter-egg, red talk |
| Arms | Dual SG90/MG90S servos with motion sequences + MCP cloud tools |
| Cloud | MCP tools for expression, page switch, arm pose, motion play |
...
- **Servos:** 2× SG90/MG90S on P18 (left) and P24 (right), external 5 V supply, shared GND
...
### 3. Apply the display patch

Landscape orientation for this enclosure requires a one-line change in the LVGL port layer (outside this app):
...
Target file: `src/liblvgl/v9/port/lv_port_disp_full_frame.c` — flips the software coordinate map 180°. If your panel is mounted the other way, skip this patch or revert to `px = 299 - ly; py = lx;`.
...
3. **Feishu calendar (optional):** edit `src/ui/feishu_cal.h` — set `FEISHU_APP_ID` and `FEISHU_APP_SECRET`.
...
**Config trap:** `tos.py build` reads `.build/cache/using.config`, not the template in `config/` directly. After changing Kconfig values, run `tos.py config choice` again or verify:
...
├── patches/          TuyaOpen SDK patches (display rotation)
├── src/
